...
if platform == 'linux':
    with open('data_exp2.csv', newline='') as f:
        data_exp1 = np.array(list(csv.reader(f))[1:], dtype=np.float32)
else:
    with open(r'C:\Users\cisco\Downloads\data_exp2.csv', newline='') as f:
        data_exp1 = np.array(list(csv.reader(f))[1:], dtype=np.float32)
...
fitparams, paramErrors = show_with_trend(t_exp1, s_exp1, 0.1, poly2, fig0, 111, f"Deslocamento X Tempo", lambda T: G*0.5*(T**2)+SO+(VO*T))

fitparams[0] *= 2
paramErrors[0] *= 2
reference = [G, VO, SO]
referenceErrors = [0.001, 0.5486, 0.1]
for i in range(3):
...
for p in [221,222,223,224]:
    ...
    fitparams, paramErrors = show_with_trend(t_exp1, v_exp1, err_v_exp1, poly1, fig1, p, f"Velocidade X Tempo (derivada com j = {j})", lambda T: T*G + VO)
    for i in range(2):
        z = zscore(fitparams[i], reference[i], paramErrors[i], referenceErrors[i])
...
new_vel = []
for i in range(4):
    new_vel.append(vel[i][:-1])
new_vel_err = []
for i in range(4):
    new_vel_err.append(vel_err[i][:-1])
new_t_exp1 = t_exp1[:-1]
for i,v in enumerate(new_vel):
    cin = 0.5*M[0]*0.001*(v/100)**2
    cin_err = (abs((v/100)**2)/2)*M[1]*0.001 + abs(M[0]*0.001*(v/100))*(new_vel_err[i]/100)
    fitparams, paramErrors = show_with_trend(new_t_exp1, cin, cin_err, poly2, fig3, 221+i, f"Energia Cinética X Tempo (velocidade com j = {i+1})", lambda T: 0.5*M[0]*0.001*((VO + G*T)*0.01)**2)
    reference = [2.404259605, 0.876717451, 3.196965456]
    referenceErrors = [0.069378833, 0.004444445, 0.019618001]
    for i in range(3):
        z = zscore(fitparams[i], reference[i], paramErrors[i], referenceErrors[i])
...
for i,a in enumerate(accel):
    ...
    fitparams, paramErrors = show_with_trend(t_exp1[:-1], pot[:-1], pot_err[:-1], poly2, fig4, 221+i, f"Energia Potencial X Tempo (aceleração com j = {i+1})",
                                             lambda T: 0.2404259605-(M[0]*0.001 * G*0.01 * ((0.5 * G*0.01 * T**2)+(VO*0.01 * T)+(SO*0.01))))
    potenciais.append(pot)
    potenciais_err.append(pot_err)

    reference = [-2.404259605, -0.876717451, 0.251883846]
    referenceErrors = [0.069378833,  0.004445, 0.001265184]
    #reference = [-2.404259605, 0, -1]
    #referenceErrors = [0.069378833, 0, 0.05]
    for i in range(3):
        z = zscore(fitparams[i], reference[i], paramErrors[i], referenceErrors[i])
        print(f"POTENTIAL with {param[i]} measurement {zscore_compatible(z)}")
...
fitparams, paramErrors = show_with_trend(t_exp1[:-1], potenciais[0][:-1] + cineticas[0], potenciais_err[0][:-1] + cineticas_err[0], poly0, fig5, 111, f"Energia Mecânica X Tempo", lambda T: np.ones(T.shape)*.2404259605)
reference = [0.259876225]
referenceError = 0.0007651837
z = zscore(fitparams[0], reference, paramErrors[0], referenceError)
print(f"MECÂNICA with REFERENCE measurement {zscore_compatible(z)}")

if platform == 'linux':
    direct = '/home/joao/Documents/ufba/eletromag/queda_livre/img_exp2/'
